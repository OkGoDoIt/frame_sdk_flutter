...
class Files {
  ...
  Future<void> writeFile(String path, dynamic data, {bool checked = false}) async {
    if (data is String) {
  ...
  Future<void> _writeFileRawBytes(String path, Uint8List data, {bool checked = false}) async {
    ...
    if (checked && callbackResponse != "c") {
      throw Exception('Couldn\'t register callback for writing to file "$path": $callbackResponse');
    }

    int currentIndex = 0;
    while (currentIndex < data.length) {
      final int maxPayload = await frame.bluetooth.maxDataPayload() - 1;
      final int nextChunkLength = data.length - currentIndex > maxPayload ? maxPayload : data.length - currentIndex;
      if (nextChunkLength == 0) break;

      if (nextChunkLength <= 0) {
        throw Exception("MTU too small to write file, or escape character at end of chunk");
      }

      final Uint8List chunk = data.sublist(currentIndex, currentIndex + nextChunkLength);
    ...
    final String? closeResponse = await frame.runLua('w:close();print("c")', awaitPrint: checked);
  ...
  Future<Uint8List> readFile(String path) async {
    ...
    return result.isNotEmpty ? result.sublist(0, result.length - (result.last == 10 ? 1 : 0)) : result;
  }
}